...
class Dashboard:
    def __init__(self,root,camera=0,rb3=True):
        ...
        root.geometry("1180x820");root.minsize(900,680);root.configure(bg="#edf2f7")
        ...
        shell=ttk.Frame(root,padding=12);shell.pack(fill="both",expand=True)
        ...
        ttk.Label(footer,textvariable=self.notice).pack(side="left")
        ttk.Button(footer,text="Buka Excel (OCR)",command=self.open_excel).pack(side="right",padx=5)
        ttk.Button(footer,text="Folder Keputusan",command=self.open_folder).pack(side="right")
